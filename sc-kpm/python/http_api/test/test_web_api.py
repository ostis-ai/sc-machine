...
class WsJsonApiTest(testing.AsyncTestCase):
  ...
  @testing.gen_test
  def test_template_search_optional(self):
    client = yield self.make_connection()
    self.assertIsNotNone(client)

    # create test construction
    params = [
        {
            'type': ScType.NodeConst
        },
    ...
    result = result['payload']
    self.assertEqual(len(result['addrs']), 0)

    # should be found with optional
    templ = [
        [
            ScAddr(elements[0]),
            [ScType.EdgeAccessVarPosTemp, "_edge"],
    ...
    result = result['payload']
    self.assertEqual(len(result['addrs']), 1)
