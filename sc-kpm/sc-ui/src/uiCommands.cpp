...
sc_result ui_command_generate_instance(sc_event_subscription const *, sc_addr arg)
{
  ...
  if (!isValid)
  {
    ...
    SYSTEM_ELEMENT(s_default_ctx, arc_addr);
  ...
  SYSTEM_ELEMENT(s_default_ctx, created_instance_addr);
    ...
    if (sc_helper_check_arc(s_default_ctx, keynode_user, author_addr, sc_type_const_perm_pos_arc) == SC_TRUE)
    {
      ...
      if (sc_iterator5_next(it5) == SC_TRUE)
      {
        ...
        while (sc_iterator3_next(it3) == SC_TRUE)
        {
          ...
          SYSTEM_ELEMENT(s_default_ctx, trans_command_addr);
...
sc_result ui_initialize_commands()
{
  /*event_ui_start_result_translation = sc_event_subscription_new(keynode_action_finished,
    sc_event_after_generate_outgoing_arc, 0, ui_start_result_translation, 0); if (event_ui_start_result_translation ==
    null) return SC_RESULT_ERROR;*/
  ...
  /*event_ui_remove_displayed_result = sc_event_subscription_new(keynode_displayed_result,
    sc_event_after_generate_outgoing_arc, 0, ui_remove_displayed_result, 0); if (event_ui_remove_displayed_result ==
    null) return SC_RESULT_ERROR;*/
