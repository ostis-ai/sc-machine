...
sc_result agent_garbage_delete(const sc_event * event, sc_addr arg)
{
  ...
  if (sc_memory_get_arc_end(s_garbage_ctx, arg, &question_addr) != SC_RESULT_OK)
    return SC_RESULT_ERROR_INVALID_STATE;

  if (sc_helper_check_arc(s_garbage_ctx, keynode_question_erase_element, question_addr, sc_type_arc_pos_const_perm) ==
      SC_FALSE)
