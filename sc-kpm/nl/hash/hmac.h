...
/// Buffer must have a size equal to HashMethod::HasBytes
template <typename HashMethod>
void hmac(const void* key, size_t numKeyBytes, const void* data, size_t numDataBytes, unsigned char * buffer)
{
  ...
  finalHasher.getHash(buffer);
}


/// convenience function for std::string
template <typename HashMethod>
void hmac(const std::string & key, const std::string & data, unsigned char * buffer)
{
	return hmac<HashMethod>(key.c_str(), key.size(), data.c_str(), data.size(), buffer);
}
