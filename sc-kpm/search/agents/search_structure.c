...
sc_result agent_search_decomposition(sc_event *event, sc_addr arg)
{
    ...
    // check question type
    if (sc_helper_check_arc(search_keynode_question_decomposition, question, sc_type_arc_pos_const_perm) == SC_FALSE)
    ...
    if (sc_iterator3_next(it1) == SC_TRUE)
    {
        ...
        it5 = sc_iterator5_a_a_f_a_f_new(sc_type_node | sc_type_const,
                                         ...
                                         search_keynode_nrel_decomposition);
    ...
    if (found == SC_TRUE)
        sc_memory_arc_new(sc_type_arc_pos_const_perm, answer, search_keynode_nrel_decomposition);
