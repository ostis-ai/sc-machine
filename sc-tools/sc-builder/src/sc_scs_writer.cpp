 ...
 */
     ...
     std::unordered_set<SCgElementPtr> & writtenElements)
 {
   ...
   for (auto const & node : allNodes)
   {
     ...
     if (identifier.empty()) identifier = NODE + UNDERSCORE + node->GetId();
     ...
     if (elementTypeStr.empty()) elementTypeStr = NODE + UNDERSCORE;
     ...
     if (link && link->GetContentType() != NO_CONTENT)
     {
       ...
       if (contentType == 1 || contentType == 2 || contentType == 3) // STRING, INTEGER, FLOAT
       {
         ...
         if (!content.empty())
         {
           ...
           buffer.AddTabs(depth + 1) << SPACE << EQUAL << SPACE << OPEN_BRACKET << content << CLOSE_BRACKET + ELEMENT_END + NEWLINE;
         }
       }
       else if (contentType == 4) // IMAGE
       {
         ...
         if (!fileName.empty() && !content.empty())
         {
           ...
           buffer.AddTabs(depth + 1) << EQUAL + SPACE + DOUBLE_QUOTE << fileContent << DOUBLE_QUOTE + ELEMENT_END + NEWLINE;
           buffer.AddTabs(depth + 1) << FORMAT_ARC + SPACE + EQUAL + SPACE + OPEN_PARENTHESIS << identifier << SPACE + SC_CONNECTOR_DCOMMON_R + SPACE << imageFormat << CLOSE_PARENTHESIS + ELEMENT_END + NEWLINE;
           buffer.AddTabs(depth + 1) << NREL_FORMAT_ARC << SPACE + EQUAL + SPACE << OPEN_PARENTHESIS + NREL_FORMAT + SPACE << SC_CONNECTOR_MAIN_R + SPACE << FORMAT_ARC + CLOSE_PARENTHESIS + ELEMENT_END + NEWLINE;
         }
       }
     }
     buffer << NEWLINE;
   ...
   for (auto const & [id, element] : elements)
   {
     if (element->GetTag() == ARC || element->GetTag() == PAIR)
     {
       ...
       if (sourceId.empty()) sourceId = NODE + UNDERSCORE + connector->GetSource()->GetId();
       if (targetId.empty()) targetId = NODE + UNDERSCORE + connector->GetTarget()->GetId();
       ...
       if (complexArcs.count(element))
       {
         ...
         for (auto const & [incomingId, incomingElement] : elements)
         {
           if (incomingElement->GetTag() == ARC || incomingElement->GetTag() == PAIR)
           {
             ...
             if (incConnector->GetTarget() == element)
             {
               ...
               if (attrSourceId.empty()) attrSourceId = NODE + UNDERSCORE + incConnector->GetSource()->GetId();
               break;
             }
           }
         }
         ...
         if (!attrSourceId.empty())
         {
           ...
           if (connectorSymbol.empty()) connectorSymbol = SC_CONNECTOR_MAIN_R;
           buffer.AddTabs(depth) << sourceId << SPACE << connectorSymbol << SPACE << attrSourceId << COLON << SPACE << targetId << ELEMENT_END << NEWLINE << NEWLINE;
         }
         else
         {
           std::string connectorSymbol;
           SCgToSCsTypesConverter::ConvertSCgConnectorTypeToSCsConnectorDesignation(connector->GetType(), connectorSymbol);
           if (connectorSymbol.empty()) connectorSymbol = SC_CONNECTOR_MAIN_R;
           buffer.AddTabs(depth) << sourceId << SPACE << connectorSymbol << SPACE << targetId << ELEMENT_END << NEWLINE << NEWLINE;
         }
       }
       else
       {
   ...
   for (auto const & [id, element] : elements)
   {
     if (element->GetTag() == CONTOUR)
     {
       ...
       if (identifier.empty()) identifier = CONTOUR + UNDERSCORE + element->GetId();
       ...
       {
         if (nodesInArcs.count(node)) continue; // Пропускаем узлы, которые будут в дугах
         std::string nodeIdentifier = node->GetIdentifier();
         if (nodeIdentifier.empty()) nodeIdentifier = NODE + UNDERSCORE + node->GetId();
