 ...
 * @param my_self_addr sc_addr representing the identity of the system.
...
/*! Function that registers event subscriptions for user authorization and deauthorization.
 * @param manager Pointer to the memory context manager for which events are registered.
 * @note This function sets up event subscriptions for user authorization and deauthorization events.
 */
void _sc_memory_context_manager_register_user_events(sc_memory_context_manager * manager);

/*! Function that unregisters event subscriptions for user authorization and deauthorization.
 * @param manager Pointer to the memory context manager for which events are unregistered.
 * @note This function releases resources associated with event subscriptions for user authorization and
 * deauthorization.
 */
...
/*! Function that resolves a memory context for a specified user by system identifier, creating a new one if it does not
 * exist.
 * @param manager Pointer to the memory context manager responsible for context resolution.
 * @param user_system_idtf System identifier of the user for whom the context is resolved.
 * @returns Returns a pointer to the resolved memory context. If an error occurs during resolution, returns null_ptr.
 * @note This function resolves a memory context for the specified user by system identifier. If the context does not
 ...
 * @param manager Pointer to the memory context manager responsible for context authorization checks.
 * @param ctx Pointer to the memory context to be checked for authorization.
