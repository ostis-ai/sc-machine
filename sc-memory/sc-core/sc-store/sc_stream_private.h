 ...
 */
typedef sc_result (*fStreamRead)(sc_stream const * stream, sc_char * data, sc_uint32 length, sc_uint32 * bytes_read);
 ...
 */
typedef sc_result (
    *fStreamWrite)(sc_stream const * stream, sc_char * data, sc_uint32 length, sc_uint32 * bytes_written);

/*! Pointer to stream seek function. This function to seek data in stream from \i origin with \i offset.
 */
typedef sc_result (*fStreamSeek)(sc_stream const * stream, sc_stream_seek_origin origin, sc_uint32 offset);

/*! Pointer to stream tell function. This function return \i position in file
 */
typedef sc_result (*fStreamTell)(sc_stream const * stream, sc_uint32 * position);

/*! Pointer to stream eof function. This function return SC_TRUE, if current position in stream is at the end
 */
typedef sc_bool (*fStreamEof)(sc_stream const * stream);

/*! Pointer to stream handler free function. This function destroys stream handler.
 */
typedef sc_result (*fStreamFreeHandler)(sc_stream const * stream);
