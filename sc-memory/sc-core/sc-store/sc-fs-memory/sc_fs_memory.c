...
{
  return manager->get_link_hashes_by_string(manager->fs_memory, string, string_size, filter);
}
    ...
    sc_link_filter * link_filter)
{
  return manager->get_link_hashes_by_substring(
      manager->fs_memory, substring, substring_size, max_length_to_search_as_prefix, link_filter);
}
    ...
    sc_link_filter * filter)
{
  return manager->get_strings_by_substring(
      manager->fs_memory, substring, substring_size, max_length_to_search_as_prefix, filter);
}
