    ...
    sc_addr const end_addr)
{
  ...
  sc_element_data *new_beg_ver = null_ptr, *new_end_ver = null_ptr;
  ...
  sc_storage_make_elements_incident_to_arc(
      connector_addr, arc_el, beg_addr, new_beg_ver, end_addr, new_end_ver, SC_FALSE, !is_not_loop);
  if (is_edge && is_not_loop)
    sc_storage_make_elements_incident_to_arc(
        connector_addr, arc_el, end_addr, new_end_ver, beg_addr, new_beg_ver, SC_TRUE, SC_FALSE);
  ...
  if (sc_type_is_structure_and_arc(new_beg_ver->flags.type, type))
    sc_storage_update_structure_arcs(connector_addr, arc_el, beg_addr, end_addr, new_end_ver);
#endif
