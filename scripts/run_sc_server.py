...
def bool_handler(run_command, flag) -> None:
    run_command = " ".join([run_command, '--' + flag])


def main(args: dict):
    run_command = " ".join([join(ostis_path, 'bin/sc-server')])
    handlers = {"extensions_path": path_handler, "repo_path": path_handler, "config": path_handler,
                "verbose": bool_handler, "clear": bool_handler}
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--extensions_path', help='Path to directory with sc-memory extensions',
                        dest='extensions')
    parser.add_argument('-r', '--repo_path', help='Path to kb.bin', dest='kb')
    ...
    parser.add_argument('-c', '--config',
                        help='Path to configuration file (Note: config file has lower priority than flags!)',
                        required=True)
